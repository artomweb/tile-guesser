...
function setup() {
  ...
  button.position(200 - button.width/2, 525);
...
class box{
  constructor(i,j){
    ...
    this.realPos = createVector(this.gridPos.x * boxWidth + boxWidth / 2, this.gridPos.y * boxHeight + boxHeight / 2);
  ...
  draw(){
    ...
    if (this.flipped){
      fill(this.hiddenCol);
      rect(this.realPos.x, this.realPos.y, this.width-10, this.height-10, 5);
    }else{
      fill(this.col);
